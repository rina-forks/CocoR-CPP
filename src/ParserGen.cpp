...
int ParserGen::GenCodeRREBNF (const Node *p, int depth) {
      ...
      case NodeType::clas:
        fputws(_SC(" "), gen);
        {
        	const CharSet* set = tab->CharClassSet(p->val);
        	fputws(_SC("["), gen);
        	for (const CharSet::Range* range = set->head; range != NULL; range = range->next) {
        		if (range->from == range->to) {
        			escapeChar(gen, range->from, escapes);
        		} else {
        			escapeChar(gen, range->from, escapes);
        			fputwc(_SC('-'), gen);
        			escapeChar(gen, range->to, escapes);
        		}
        	}
        	fputws(_SC("]"), gen);
        	++rc;
...
void ParserGen::WriteRREBNF () {
	...
	for (int i=0; i<tab->terminals.Count; i++) {
		sym = tab->terminals[i];
                if (isalpha((int)sym->name[0])) {

								if (sym->tokenKind == Symbol::classToken || sym->tokenKind == Symbol::classLitToken) {
                fwprintf(gen, "%" _SFMT " ::= ", sym->name);
                if(GenCodeRREBNF(sym->graph, 0) == 0) {
                        fputws(_SC("\"\?\?()\?\?\""), gen);
                }
